...
const LocationList = () => {
  ...
  const filters = ui.filters.locations || {};
  const search = filters.search || '';
  const showInactive = Boolean(filters.showInactive);

  const setFilter = useCallback(
    (partial) => ui.setFilter('locations', partial),
    [ui]
  );

  const locations = siteConfig.data.locations || [];
  ...
  const handleDelete = useCallback(
    (row) => {
      openConfirm({
        title: 'Xóa địa điểm',
        message: `Xóa địa điểm "${row.name?.vi || row.name?.en || ''}"?`,
        confirmText: 'Xóa',
        confirmStyle: 'danger',
        onConfirm: async () => {
          try {
            addNotification('Xóa địa điểm thành công');
            siteConfig.load();
          } catch (err) {
            addNotification(
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <SEO title="Địa điểm" description="Quản lý địa điểm" url="/locations" />
          ...
          {siteConfig.loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-gray-400 gap-3">
              ...
              <p className="text-base">
                {search ? 'Không tìm thấy địa điểm phù hợp' : 'Chưa có địa điểm nào'}
              </p>
              {!search && (
                <button
                  onClick={() => navigate('/locations/new')}
