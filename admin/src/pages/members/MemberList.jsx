...
const MemberList = () => {
  ...
  // Members are pre-loaded in siteConfigSlice; fall back to direct API if not.
  ...
  const filters = ui.filters.members || {};
  const search = filters.search || '';
  const showInactive = Boolean(filters.showInactive);

  const setFilter = useCallback(
    (partial) => ui.setFilter('members', partial),
    [ui]
  );

  const members = siteConfig.data.members || [];

  // Derive filtered list from store pre-loaded data
  ...
  const handleDelete = useCallback(
    (row) => {
      openConfirm({
        ...
        onConfirm: async () => {
          try {
            const res = await useAdminStore.getState().siteConfig.load();
            // If siteConfig.deleteMember exists, call it; otherwise fall back to direct API
            addNotification('Xóa thành viên thành công');
            // Force refresh from siteConfig
            useAdminStore.getState().siteConfig.load();
          } catch (err) {
            addNotification(
        ...
        render: (val, row) => (
          <div className="min-w-0">
            <span className="font-medium text-gray-800 block">
              {val?.vi || (
                <em className="text-gray-400">(chưa có tên VI)</em>
              )}
            </span>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <SEO
        title="Ban lãnh đạo"
        description="Quản lý thành viên ban lãnh đạo"
        url="/members"
      />
      <Header title="Quản lý Ban lãnh đạo" />
