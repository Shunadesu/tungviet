...
export const leadershipService = {
  async getPublic(locale = 'vi') {
    const members = await Leadership.find({ isActive: true })
  ...
  async getById(id) {
    const member = await Leadership.findOne({ _id: id }).lean();
    if (!member) throw AppError.notFound('Khong tim thay thanh vien');
    return member;
  },

  async create(data) {
    const maxOrder = await Leadership.findOne({}, 'order').sort({ order: -1 }).lean();
    const order = typeof data.order === 'number' ? data.order : (maxOrder?.order ?? -1) + 1;

    const member = new Leadership({
  ...
  async delete(id) {
    const member = await Leadership.findOne({ _id: id });
    if (!member) throw AppError.notFound('Khong tim thay thanh vien');
    await member.delete();
    invalidatePublicCache();
    return member;
  ...
  async reorder(orderedIds) {
    const current = await Leadership.find({}).lean();
    ...
    await Promise.all(
      next.map((m) => Leadership.findByIdAndUpdate(m._id, { order: m.order }))
    );
    invalidatePublicCache();
    return next;
  },
